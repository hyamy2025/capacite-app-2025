...
export default function TDP() {
  ...
  const [specialites, setSpecialites] = useState([{ nom: '', existant: { groupes: '', apprenants: '' }, ajout: { groupes: '' }, th: '', pr: '' }]);
  ...
  const moyenneCapacite = () => moyenne(sallesTh.map(s => parseFloat(s.superficie || 0) / parseFloat(s.cno || 1)));
  const totalAjoutApprenants = (spec) => calculTotalApprenantsAjoutes(spec.ajout.groupes, moyenneCapacite());
  const totalGroupes = (spec) => parseInt(spec.existant.groupes || 0) + parseInt(spec.ajout.groupes || 0);
  const totalApprenants = (spec) => parseInt(spec.existant.apprenants || 0) + totalAjoutApprenants(spec);

  const besoinTh = (spec) => parseFloat(spec.th || 0) * totalGroupes(spec);
  const besoinPr = (spec) => parseFloat(spec.pr || 0) * totalGroupes(spec);

  const totalTh = somme(specialites.map(besoinTh));
  const totalPr = somme(specialites.map(besoinPr));
  const heuresTh = somme(sallesTh.map(s => heuresDisponibles(s.semaines)));
  const heuresPr = somme(sallesPr.map(s => heuresDisponibles(s.semaines)));

  const deltaTh = heuresTh - totalTh;
  const deltaPr = heuresPr - totalPr;
  const testGlobal = () => (deltaTh >= 0 && deltaPr >= 0 ? 'Excédent' : 'Dépassement');

  const exportPDF = () => {
    const doc = new jsPDF();
    doc.text("Rapport TDP", 10, 10);
    ...
    doc.save("resultat-tdp.pdf");
  };

  return (
    <div className="p-6 max-w-6xl mx-auto">
      <h1 className="text-2xl font-bold text-center mb-6">Test de Dépassement Prévu (TDP)</h1>
